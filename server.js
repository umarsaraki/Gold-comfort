...
async function getWalletBalance(ownerType, ownerId) {
  if (ownerType === 'reseller') {
    const r = await db.prepare('SELECT available_balance FROM resellers WHERE id = ?').get(ownerId);
    return r ? r.available_balance : 0;
  }
  const u = await db.prepare('SELECT available_balance FROM users WHERE email = ?').get(ownerId);
  return u ? u.available_balance : 0;
}
async function setWalletBalance(ownerType, ownerId, newBalance) {
  if (ownerType === 'reseller') {
    await db.prepare('UPDATE resellers SET available_balance = ? WHERE id = ?').run(newBalance, ownerId);
  } else {
    await db.prepare('UPDATE users SET available_balance = ? WHERE email = ?').run(newBalance, ownerId);
  }
}
async function creditWallet(ownerType, ownerId, amount, reason, relatedOrderId) {
  const newBalance = getWalletBalance(ownerType, ownerId) + amount;
  setWalletBalance(ownerType, ownerId, newBalance);
  if (ownerType === 'reseller') {
    await db.prepare('UPDATE resellers SET total_earning = total_earning + ? WHERE id = ?').run(amount, ownerId);
  }
  await db.prepare('INSERT INTO wallet_transactions (owner_type, owner_id, direction, amount, balance_after, reason, related_order_id) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(ownerType, String(ownerId), 'credit', amount, newBalance, reason, relatedOrderId || null);
  return newBalance;
}
async function debitWallet(ownerType, ownerId, amount, reason, relatedOrderId) {
...
async function getPendingBalance(ownerType, ownerId) {
  if (ownerType === 'reseller') {
    const row = await db.prepare(`SELECT COALESCE(SUM(reseller_payout),0) p FROM orders WHERE reseller_id = ? AND status IN ('preparing','on-way','arrived')`).get(ownerId);
    return row.p;
  }
  if (ownerType === 'admin') {
    ...
    const row = await db.prepare(`SELECT COALESCE(SUM(reseller_payout),0) p FROM orders WHERE status IN ('preparing','on-way','arrived')`).get();
    return row.p;
  }
  return 0;
}
async function getResellerEarnedTotal() {
  // Total ever actually paid out to resellers across the whole platform.
  const row = await db.prepare(`SELECT COALESCE(SUM(total_earning),0) t FROM resellers`).get();
  return row.t;
}
async function getTotalBalanceEverCredited(ownerType, ownerId) {
  const row = await db.prepare(`SELECT COALESCE(SUM(amount),0) t FROM wallet_transactions WHERE owner_type = ? AND owner_id = ? AND direction = 'credit'`).get(ownerType, String(ownerId));
  return row.t;
}
async function getTotalRefunded(ownerType, ownerId) {
  const row = await db.prepare(`SELECT COALESCE(SUM(amount),0) t FROM wallet_transactions WHERE owner_type = ? AND owner_id = ? AND direction = 'credit' AND reason LIKE 'Refund%'`).get(ownerType, String(ownerId));
  return row.t;
}
...
async function verifySecurityCode(email, code) {
  ...
  if (hash !== u.security_code_hash) {
    ...
    if (attempts >= 5) {
      ...
      await db.prepare('UPDATE users SET security_code_attempts = 0, security_code_locked_until = ? WHERE email = ?').run(lockedUntil, email);
      return { ok: false, error: 'Too many wrong attempts — locked for 15 minutes' };
    }
    await db.prepare('UPDATE users SET security_code_attempts = ? WHERE email = ?').run(attempts, email);
    return { ok: false, error: `Wrong security code (${5 - attempts} attempts left)` };
  }
  await db.prepare('UPDATE users SET security_code_attempts = 0 WHERE email = ?').run(email);
  return { ok: true };
}
...
route('GET', '/api/security-code/:email/status', async (req, res, body, params) => {
  ...
  const u = await db.prepare('SELECT security_code_hash FROM users WHERE email = ?').get(email);
  sendJSON(res, 200, { isSet: !!(u && u.security_code_hash) });
});
route('POST', '/api/security-code/:email/set', async (req, res, body, params) => {
  const email = decodeURIComponent(params.email);
  const existing = await db.prepare('SELECT security_code_hash FROM users WHERE email = ?').get(email);
  ...
  await db.prepare('UPDATE users SET security_code_hash = ?, security_code_salt = ?, security_code_attempts = 0, security_code_locked_until = NULL WHERE email = ?')
    .run(hash, salt, email);
...
route('POST', '/api/auth/login', async (req, res, body) => {
  ...
  const existing = await db.prepare('SELECT * FROM users WHERE email = ?').get(identity.email);
  if (!existing) {
    await db.prepare('INSERT INTO users (email, name, is_admin) VALUES (?, ?, ?)')
      .run(identity.email, identity.name, identity.email === ADMIN_EMAIL ? 1 : 0);
  }
  const user = await db.prepare('SELECT * FROM users WHERE email = ?').get(identity.email);
  ...
  await db.prepare('INSERT INTO sessions (token, user_email) VALUES (?, ?)').run(token, user.email);
...
route('GET', '/api/auth/me', async (req, res) => {
  ...
  const user = await db.prepare('SELECT * FROM users WHERE email = ?').get(session.user_email);
...
route('POST', '/api/auth/logout', async (req, res) => {
  const cookies = parseCookies(req);
  if (cookies.gc_session) await db.prepare('DELETE FROM sessions WHERE token = ?').run(cookies.gc_session);
...
route('GET', '/api/kyc/verified/:email', async (req, res, body, params) => {
  const row = await db.prepare('SELECT * FROM verified_identities WHERE email = ?').get(decodeURIComponent(params.email));
  sendJSON(res, row ? 200 : 404, row || { error: 'not verified yet' });
});

route('POST', '/api/kyc/verify-identity', async (req, res, body) => {
  body.email = (body.email || '').toLowerCase();
  const existing = await db.prepare('SELECT * FROM verified_identities WHERE email = ?').get(body.email);
  ...
  await db.prepare(`INSERT INTO verified_identities (email, fullname, phone, nin, selfie_photo, dob) VALUES (?, ?, ?, ?, ?, ?)`)
    .run(body.email, body.fullname, body.phone, body.nin, body.selfiePhoto || null, body.dob);
  const identity = await db.prepare('SELECT * FROM verified_identities WHERE email = ?').get(body.email);
  sendJSON(res, 201, { cached: false, identity });
});

route('DELETE', '/api/kyc/verified/:email', async (req, res, body, params) => {
  await db.prepare('DELETE FROM verified_identities WHERE email = ?').run(decodeURIComponent(params.email).toLowerCase());
...
route('POST', '/api/resellers', async (req, res, body) => {
  body.email = (body.email || '').toLowerCase();
  const verified = await db.prepare('SELECT * FROM verified_identities WHERE email = ?').get(body.email);
  ...
  const existingUser = await db.prepare('SELECT 1 FROM users WHERE email = ?').get(body.email);
  if (!existingUser) {
    await db.prepare('INSERT INTO users (email, name) VALUES (?, ?)').run(body.email, verified.fullname || body.email);
  }
  ...
  if (body.payWithWallet && body.walletAmount > 0) {
    try {
      ...
      await db.prepare('DELETE FROM resellers WHERE id = ?').run(info.lastInsertRowid);
      return sendJSON(res, 400, { error: err.message });
    }
  }

  const reseller = await db.prepare('SELECT * FROM resellers WHERE id = ?').get(info.lastInsertRowid);
  ...
  if (body.bankName && body.bankAcct && body.bankAcctName) {
    await db.prepare('INSERT INTO saved_bank_accounts (owner_type, owner_id, bank_name, bank_acct, bank_acct_name) VALUES (?, ?, ?, ?, ?)')
...
route('GET', '/api/resellers', async (req, res, body, params, query) => {
  let list = await await db.prepare('SELECT * FROM resellers ORDER BY created_at DESC').all();
  ...
  let enriched = await Promise.all(list.map(async r => {
    const salesRow = await await db.prepare("SELECT COALESCE(SUM(reseller_payout),0) s FROM orders WHERE reseller_id = ? AND status = 'delivered'").get(r.id);
    return { ...r, plan_expired: isPlanExpired(r), total_sales: salesRow.s };
  }));
...
route('GET', '/api/resellers/:id', async (req, res, body, params) => {
  const r = await db.prepare('SELECT * FROM resellers WHERE id = ?').get(params.id);
...
route('DELETE', '/api/resellers/:id', async (req, res, body, params) => {
  ...
  await db.prepare('DELETE FROM listings WHERE reseller_id = ?').run(params.id);
  await db.prepare('DELETE FROM saved_bank_accounts WHERE owner_type = ? AND owner_id = ?').run('reseller', String(params.id));
  await db.prepare('DELETE FROM virtual_accounts WHERE owner_type = ? AND owner_id = ?').run('reseller', String(params.id));
  await db.prepare('DELETE FROM resellers WHERE id = ?').run(params.id);
...
route('POST', '/api/resellers/:id/extend', async (req, res, body, params) => {
  ...
  if (body.plan) {
    await db.prepare('UPDATE resellers SET plan_expires_at = ?, plan = ?, interest_while_expired = 0 WHERE id = ?').run(newExpiry, body.plan, params.id);
  } else {
    await db.prepare('UPDATE resellers SET plan_expires_at = ?, interest_while_expired = 0 WHERE id = ?').run(newExpiry, params.id);
  }
  sendJSON(res, 200, { plan_expires_at: newExpiry });
});

route('POST', '/api/resellers/:id/register-interest', async (req, res, body, params) => {
  await db.prepare('UPDATE resellers SET interest_while_expired = interest_while_expired + 1 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { ok: true });
});

route('POST', '/api/resellers/:id/ban', async (req, res, body, params) => {
  await db.prepare('UPDATE resellers SET banned = 1 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { banned: true });
});
route('POST', '/api/resellers/:id/unban', async (req, res, body, params) => {
  await db.prepare('UPDATE resellers SET banned = 0 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { banned: false });
});

// -- Customers (non-reseller users) --
route('GET', '/api/customers', async (req, res, body, params, query) => {
  const resellerRows = await await db.prepare('SELECT user_email FROM resellers').all();
  const resellerEmails = new Set(resellerRows.map(r => r.user_email));
  const allUsers = await await db.prepare('SELECT * FROM users').all();
  let list = allUsers.filter(u => !resellerEmails.has(u.email));
  if (query.search) {
    const term = query.search.toLowerCase();
    list = list.filter(u => u.name.toLowerCase().includes(term) || u.email.toLowerCase().includes(term));
  }
  let enriched = await Promise.all(list.map(async u => {
    const spentRow = await await db.prepare("SELECT COALESCE(SUM(total),0) s FROM orders WHERE buyer_email = ? AND status = 'delivered'").get(u.email);
    return { ...u, total_spent: spentRow.s };
  }));
  if (query.sort === 'amount') {
...
route('POST', '/api/customers/:email/ban', async (req, res, body, params) => {
  await db.prepare('UPDATE users SET banned = 1 WHERE email = ?').run(decodeURIComponent(params.email));
  sendJSON(res, 200, { banned: true });
});
route('POST', '/api/customers/:email/unban', async (req, res, body, params) => {
  await db.prepare('UPDATE users SET banned = 0 WHERE email = ?').run(decodeURIComponent(params.email));
  sendJSON(res, 200, { banned: false });
});
...
route('POST', '/api/listings', async (req, res, body) => {
  const reseller = await db.prepare('SELECT * FROM resellers WHERE id = ?').get(body.resellerId);
...
route('GET', '/api/listings', async (req, res, body, params, query) => {
  let list = await db.prepare('SELECT * FROM listings ORDER BY created_at DESC').all();
  ...
  const enrichedList = await Promise.all(list.map(async l => {
    const reseller = await db.prepare('SELECT * FROM resellers WHERE id = ?').get(l.reseller_id);
    return {
      ...serializeListing(l),
  ...
  const enriched = enrichedList.filter(l => !l.seller_banned);
...
route('PATCH', '/api/listings/:id', async (req, res, body, params) => {
  const l = await db.prepare('SELECT * FROM listings WHERE id = ?').get(params.id);
  ...
  await db.prepare(`UPDATE listings SET name=?, description=?, price_text=?, photos=?, viewers=?, rating=?, discount_code=?, discount_percent=? WHERE id=?`)
  ...
  const updated = await db.prepare('SELECT * FROM listings WHERE id = ?').get(params.id);
...
route('POST', '/api/orders', async (req, res, body) => {
  const listing = await db.prepare('SELECT * FROM listings WHERE id = ?').get(body.listingId);
  if (!listing) return sendJSON(res, 404, { error: 'listing not found' });
  const reseller = await db.prepare('SELECT * FROM resellers WHERE id = ?').get(listing.reseller_id);
  ...
  for (const cid of creditIds) {
    const c = await db.prepare('SELECT * FROM customer_credits WHERE id = ? AND customer_email = ? AND used = 0').get(cid, body.buyerEmail);
  ...
  if (body.shopCouponCode) {
    const coupon = await db.prepare("SELECT * FROM coupons WHERE code = ? AND active = 1 AND type = 'shop'").get(body.shopCouponCode.trim().toUpperCase());
  ...
  for (const cid of appliedCredits) {
    await db.prepare('UPDATE customer_credits SET used = 1, used_order_id = ? WHERE id = ?').run(info.lastInsertRowid, cid);
  }
  ...
  if (body.payWithWallet && finalTotal > 0) {
    try {
      ...
      await db.prepare("DELETE FROM orders WHERE id = ?").run(info.lastInsertRowid);
      return sendJSON(res, 400, { error: err.message });
    }
  }
...
route('GET', '/api/orders', async (req, res, body, params, query) => {
  let list = await db.prepare('SELECT * FROM orders ORDER BY ordered_at DESC').all();
  ...
  const enriched = await Promise.all(list.map(async o => {
    const listing = await db.prepare('SELECT name, type FROM listings WHERE id = ?').get(o.listing_id);
    const reseller = await db.prepare('SELECT biz_name, user_email FROM resellers WHERE id = ?').get(o.reseller_id);
    const identity = reseller ? await db.prepare('SELECT phone FROM verified_identities WHERE email = ?').get(reseller.user_email) : null;
...
route('POST', '/api/orders/:id/accept', async (req, res, body, params) => {
  ...
  await db.prepare("UPDATE orders SET status = 'preparing', expected_delivery_at = ? WHERE id = ? AND status = 'pending'").run(body.expectedDeliveryAt, params.id);
...
route('POST', '/api/orders/:id/reject', async (req, res, body, params) => {
  ...
  await db.prepare("UPDATE orders SET status = 'seller-declined', decline_reason = ? WHERE id = ?").run(body.reason.trim(), params.id);
...
route('POST', '/api/orders/:id/advance', async (req, res, body, params) => {
  ...
  await db.prepare('UPDATE orders SET status = ? WHERE id = ?').run(next, params.id);
...
route('POST', '/api/orders/:id/confirm', async (req, res, body, params) => {
  const order = await db.prepare('SELECT * FROM orders WHERE id = ?').get(params.id);
  if (!order) return sendJSON(res, 404, { error: 'not found' });
  await db.prepare("UPDATE orders SET status = 'delivered', delivered_at = NOW() WHERE id = ?").run(params.id);
  await creditWallet('reseller', order.reseller_id, order.reseller_payout, `Order #${order.id} delivered`, order.id);
  await creditWallet('admin', ADMIN_EMAIL, order.commission, `Commission — order #${order.id}`, order.id);
  ...
  const cashbackPercent = Number(await getSetting('cashback_percent', '1'));
  const cashbackExpiryDays = Number(await getSetting('cashback_expiry_days', '180'));
  ...
  if (cashbackPercent > 0) {
    ...
    if (cashbackAmount > 0) {
      ...
      await db.prepare('INSERT INTO customer_credits (customer_email, amount, reason, expires_at) VALUES (?, ?, ?, ?)')
...
async function checkExpiredCredits() {
  await db.prepare(`UPDATE customer_credits SET used = 1 WHERE used = 0 AND expires_at IS NOT NULL AND expires_at < NOW()`).run();
}
...
route('POST', '/api/orders/:id/chat', async (req, res, body, params) => {
  await db.prepare('INSERT INTO order_chat (order_id, from_role, text) VALUES (?, ?, ?)').run(params.id, body.fromRole, body.text);
  sendJSON(res, 201, { ok: true });
});
route('GET', '/api/orders/:id/chat', async (req, res, body, params) => {
  const messages = await db.prepare('SELECT * FROM order_chat WHERE order_id = ? ORDER BY at ASC').all(params.id);
...
route('POST', '/api/buy-requests', async (req, res, body) => {
  const reseller = await db.prepare('SELECT * FROM resellers WHERE id = ?').get(body.resellerId);
  if (!reseller) return sendJSON(res, 404, { error: 'reseller not found' });
  const info = await db.prepare('INSERT INTO buy_requests (reseller_id, item_name, description) VALUES (?, ?, ?) RETURNING id')
    .run(reseller.id, body.itemName, body.description || '');
  const br = await db.prepare('SELECT * FROM buy_requests WHERE id = ?').get(info.lastInsertRowid);
  sendJSON(res, 201, { buyRequest: br });
});
route('GET', '/api/buy-requests', async (req, res, body, params, query) => {
  let list = await db.prepare('SELECT * FROM buy_requests WHERE closed = 0 ORDER BY created_at DESC').all();
  const enrichedList = await Promise.all(list.map(async br => {
    const reseller = await db.prepare('SELECT biz_name, user_email, banned FROM resellers WHERE id = ?').get(br.reseller_id);
    const msgCount = (await db.prepare('SELECT COUNT(*) c FROM buy_request_chat WHERE buy_request_id = ?').get(br.id)).c;
    const lastMessage = await db.prepare('SELECT * FROM buy_request_chat WHERE buy_request_id = ? ORDER BY at DESC LIMIT 1').get(br.id);
    return {
      ...br,
  ...
  const enriched = enrichedList.filter(br => !br.seller_banned);
  sendJSON(res, 200, { buyRequests: enriched });
});
route('POST', '/api/buy-requests/:id/close', async (req, res, body, params) => {
  await db.prepare('UPDATE buy_requests SET closed = 1 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { closed: true });
});
route('POST', '/api/buy-requests/:id/message', async (req, res, body, params) => {
  await db.prepare('INSERT INTO buy_request_chat (buy_request_id, from_email, text) VALUES (?, ?, ?)').run(params.id, body.fromEmail, body.text);
  sendJSON(res, 201, { ok: true });
});
route('GET', '/api/buy-requests/:id/thread', async (req, res, body, params) => {
  const messages = await db.prepare('SELECT * FROM buy_request_chat WHERE buy_request_id = ? ORDER BY at ASC').all(params.id);
  sendJSON(res, 200, { messages });
});

// -- Reports (abuse anywhere — Buy Requests, chat, orders) --
route('POST', '/api/reports', async (req, res, body) => {
  await db.prepare('INSERT INTO reports (reporter_email, reported_email, reason, context) VALUES (?, ?, ?, ?)')
...
route('POST', '/api/reports/:id/resolve', async (req, res, body, params) => {
  await db.prepare('UPDATE reports SET resolved = 1 WHERE id = ?').run(params.id);
...
route('GET', '/api/coupons', async (req, res, body, params, query) => {
  let list = await db.prepare('SELECT * FROM coupons WHERE active = 1 ORDER BY created_at DESC').all();
...
route('GET', '/api/coupons/:code/check', async (req, res, body, params, query) => {
  const c = await db.prepare('SELECT * FROM coupons WHERE code = ? AND active = 1').get(decodeURIComponent(params.code).trim().toUpperCase());
...
route('POST', '/api/coupons/:id/deactivate', async (req, res, body, params) => {
  await db.prepare('UPDATE coupons SET active = 0 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { ok: true });
});

// -- Home banner slides (admin-managed carousel — image, text, or both) --
route('POST', '/api/banner-slides', async (req, res, body) => {
  const maxOrder = (await db.prepare('SELECT MAX(sort_order) m FROM banner_slides').get())?.m || 0;
  const info = await db.prepare('INSERT INTO banner_slides (image, text, link_url, sort_order) VALUES (?, ?, ?, ?) RETURNING id')
    .run(body.image || null, body.text || null, body.linkUrl || null, maxOrder + 1);
  sendJSON(res, 201, { id: info.lastInsertRowid });
});
route('GET', '/api/banner-slides', async (req, res) => {
  const slides = await db.prepare('SELECT * FROM banner_slides ORDER BY sort_order ASC').all();
  sendJSON(res, 200, { slides });
});
route('DELETE', '/api/banner-slides/:id', async (req, res, body, params) => {
  await db.prepare('DELETE FROM banner_slides WHERE id = ?').run(params.id);
  sendJSON(res, 200, { deleted: true });
});
route('POST', '/api/banner-slides/:id/view', async (req, res, body, params) => {
  await db.prepare('UPDATE banner_slides SET views = views + 1 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { ok: true });
});
route('POST', '/api/banner-slides/:id/click', async (req, res, body, params) => {
  await db.prepare('UPDATE banner_slides SET clicks = clicks + 1 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { ok: true });
});

// -- Customer credits (ad-hoc "ragowa" from a reseller, spendable anywhere) --
route('POST', '/api/credits', async (req, res, body) => {
  const info = await db.prepare('INSERT INTO customer_credits (customer_email, amount, granted_by_reseller_id, reason) VALUES (?, ?, ?, ?) RETURNING id')
    .run(body.customerEmail, body.amount, body.resellerId || null, body.reason || '');
  sendJSON(res, 201, { id: info.lastInsertRowid });
});
route('GET', '/api/credits', async (req, res, body, params, query) => {
  let list = await db.prepare('SELECT * FROM customer_credits WHERE customer_email = ? ORDER BY created_at DESC').all(query.email || '');
...
route('GET', '/api/settings/:key', async (req, res, body, params) => {
  const row = await db.prepare('SELECT value FROM settings WHERE "key" = ?').get(params.key);
  sendJSON(res, 200, { value: row ? row.value : null });
});
route('POST', '/api/settings/:key', async (req, res, body, params) => {
  await db.prepare('INSERT INTO settings ("key", value) VALUES (?, ?) ON CONFLICT ("key") DO UPDATE SET value = EXCLUDED.value').run(params.key, body.value);
  sendJSON(res, 200, { ok: true });
});

// -- Notifications --
route('POST', '/api/notifications', async (req, res, body) => {
  const info = await db.prepare('INSERT INTO notifications (message, audience) VALUES (?, ?) RETURNING id').run(body.message, body.audience);
  sendJSON(res, 201, { id: info.lastInsertRowid });
});
route('GET', '/api/notifications', async (req, res, body, params, query) => {
  const email = query.email;
  const isReseller = email ? !!await db.prepare('SELECT 1 FROM resellers WHERE user_email = ?').get(email) : false;
  let list = await db.prepare('SELECT * FROM notifications ORDER BY sent_at DESC').all();
  if (query.all !== '1') {
    list = list.filter(n => n.audience === 'all' || (n.audience === 'reseller' && isReseller) || n.audience === 'customer');
  }
  const reads = new Set((await db.prepare('SELECT notification_id FROM notification_reads WHERE user_email = ?').all(email)).map(r => r.notification_id));
...
route('POST', '/api/notifications/:id/read', async (req, res, body, params) => {
  await db.prepare('INSERT OR IGNORE INTO notification_reads (notification_id, user_email) VALUES (?, ?)').run(params.id, body.email);
  sendJSON(res, 200, { ok: true });
});
...
route('POST', '/api/support/:email/message', async (req, res, body, params) => {
  const email = decodeURIComponent(params.email);
  await db.prepare('INSERT INTO support_messages (user_email, from_role, text) VALUES (?, ?, ?)').run(email, body.fromRole, body.text);
  await db.prepare('INSERT INTO support_threads (user_email) VALUES (?) ON CONFLICT (user_email) DO NOTHING').run(email);
  if (body.fromRole === 'user') await db.prepare('UPDATE support_threads SET unread_for_admin = 1 WHERE user_email = ?').run(email);
  if (body.fromRole === 'admin') await db.prepare('UPDATE support_threads SET unread_for_user = 1, escalated = 1 WHERE user_email = ?').run(email);
  sendJSON(res, 201, { ok: true });
});
route('POST', '/api/support/:email/escalate', async (req, res, body, params) => {
  const email = decodeURIComponent(params.email);
  await db.prepare('INSERT INTO support_threads (user_email) VALUES (?) ON CONFLICT (user_email) DO NOTHING').run(email);
  await db.prepare('UPDATE support_threads SET escalated = 1, unread_for_admin = 1 WHERE user_email = ?').run(email);
  sendJSON(res, 200, { ok: true });
});
route('GET', '/api/support/:email/thread', async (req, res, body, params) => {
  const email = decodeURIComponent(params.email);
  const messages = await db.prepare('SELECT * FROM support_messages WHERE user_email = ? ORDER BY at ASC').all(email);
  const thread = await db.prepare('SELECT * FROM support_threads WHERE user_email = ?').get(email) || { escalated: 0, unread_for_admin: 0, unread_for_user: 0 };
...
route('POST', '/api/support/:email/mark-read', async (req, res, body, params) => {
  ...
  await db.prepare(`UPDATE support_threads SET ${field} = 0 WHERE user_email = ?`).run(email);
  sendJSON(res, 200, { ok: true });
});
route('GET', '/api/support/inbox', async (req, res) => {
  const threads = await db.prepare('SELECT * FROM support_threads').all();
  sendJSON(res, 200, { threads });
});

// -- Admin stats --
route('GET', '/api/admin/stats', async (req, res) => {
  const totalOrders = (await db.prepare('SELECT COUNT(*) c FROM orders').get()).c;
  const completed = (await db.prepare("SELECT COUNT(*) c FROM orders WHERE status = 'delivered'").get()).c;
  const pending = (await db.prepare("SELECT COUNT(*) c FROM orders WHERE status IN ('pending','preparing','on-way','arrived')").get()).c;
  const totalResellers = (await db.prepare('SELECT COUNT(*) c FROM resellers').get()).c;
...
route('GET', '/api/plans/first-time-price', async (req, res, body, params, query) => {
  ...
  const everRegistered = query.email ? !!await db.prepare('SELECT 1 FROM resellers WHERE user_email = ?').get(query.email) : true;
...
route('POST', '/api/listings/:id/view', async (req, res, body, params) => {
  await db.prepare('UPDATE listings SET viewers = viewers + 1 WHERE id = ?').run(params.id);
  sendJSON(res, 200, { ok: true });
});
...
route('POST', '/api/listings/:id/rate', async (req, res, body, params) => {
  ...
  await db.prepare(`
    ...
    ON CONFLICT (listing_id, customer_email) DO UPDATE SET stars = EXCLUDED.stars, at = NOW()
  `).run(params.id, body.email, stars);
  const avg = await db.prepare('SELECT AVG(stars) a, COUNT(*) c FROM listing_ratings WHERE listing_id = ?').get(params.id);
  await db.prepare('UPDATE listings SET rating = ? WHERE id = ?').run(avg.a || 0, params.id);
  sendJSON(res, 200, { rating: avg.a || 0, count: avg.c });
});

route('GET', '/api/listings/:id/rating-breakdown', async (req, res, body, params) => {
  const rows = await db.prepare('SELECT stars, COUNT(*) c FROM listing_ratings WHERE listing_id = ? GROUP BY stars').all(params.id);
...
route('POST', '/api/otp/verify', async (req, res, body) => {
  ...
  const row = await db.prepare('SELECT * FROM otp_codes WHERE phone = ?').get(body.phone);
  ...
  try {
    ...
    if (verified) await db.prepare('UPDATE otp_codes SET verified = 1 WHERE phone = ?').run(body.phone);
...
route('POST', '/api/webhooks/flutterwave', async (req, res, body) => {
  ...
  if (incomingAccountNumber) {
    const va = await db.prepare('SELECT * FROM virtual_accounts WHERE account_number = ?').get(incomingAccountNumber);
    if (va) {
...
route('GET', '/api/wallet/:ownerType/:ownerId', async (req, res, body, params) => {
  ...
  if (ownerType === 'reseller') {
    const r = await db.prepare('SELECT total_earning FROM resellers WHERE id = ?').get(ownerId);
  ...
  if (ownerType === 'customer') {
    const row = await db.prepare(`SELECT COALESCE(SUM(amount),0) c FROM customer_credits WHERE customer_email = ? AND used = 0 AND (expires_at IS NULL OR expires_at >= NOW())`).get(decodedId);
...
route('GET', '/api/wallet/:ownerType/:ownerId/transactions', async (req, res, body, params) => {
  const { ownerType, ownerId } = params;
  const txns = await db.prepare('SELECT * FROM wallet_transactions WHERE owner_type = ? AND owner_id = ? ORDER BY created_at DESC LIMIT 100')
...
route('POST', '/api/wallet/:ownerType/:ownerId/virtual-account', async (req, res, body, params) => {
  const { ownerType, ownerId } = params;
  const decodedId = decodeURIComponent(ownerId);
  const existing = await db.prepare('SELECT * FROM virtual_accounts WHERE owner_type = ? AND owner_id = ?').get(ownerType, decodedId);
  ...
  if (ownerType === 'reseller') {
    const reseller = await db.prepare('SELECT user_email FROM resellers WHERE id = ?').get(decodedId);
    const identity = reseller ? await db.prepare('SELECT fullname FROM verified_identities WHERE email = ?').get(reseller.user_email) : null;
    realName = identity?.fullname || realName;
  } else {
    const identity = await db.prepare('SELECT fullname FROM verified_identities WHERE email = ?').get(decodedId);
    const user = await db.prepare('SELECT name FROM users WHERE email = ?').get(decodedId);
  ...
  await db.prepare('INSERT INTO virtual_accounts (owner_type, owner_id, account_number, account_name, bank_name, flw_reference) VALUES (?, ?, ?, ?, ?, ?)')
    .run(ownerType, decodedId, result.accountNumber, result.accountName, result.bankName, result.flwReference || null);
  const virtualAccount = await db.prepare('SELECT * FROM virtual_accounts WHERE owner_type = ? AND owner_id = ?').get(ownerType, decodedId);
  sendJSON(res, 201, { virtualAccount });
});

async function resolveOwnerEmail(ownerType, decodedId) {
  if (ownerType === 'reseller') {
    const r = await db.prepare('SELECT user_email FROM resellers WHERE id = ?').get(decodedId);
...
route('GET', '/api/bank-accounts/:ownerType/:ownerId', async (req, res, body, params) => {
  const { ownerType, ownerId } = params;
  const accounts = await db.prepare('SELECT * FROM saved_bank_accounts WHERE owner_type = ? AND owner_id = ? ORDER BY created_at ASC').all(ownerType, decodeURIComponent(ownerId));
  sendJSON(res, 200, { accounts });
});
route('POST', '/api/bank-accounts/:ownerType/:ownerId', async (req, res, body, params) => {
  const { ownerType, ownerId } = params;
  const decodedId = decodeURIComponent(ownerId);
  const count = (await db.prepare('SELECT COUNT(*) c FROM saved_bank_accounts WHERE owner_type = ? AND owner_id = ?').get(ownerType, decodedId)).c;
  ...
  const info = await db.prepare('INSERT INTO saved_bank_accounts (owner_type, owner_id, bank_name, bank_acct, bank_acct_name) VALUES (?, ?, ?, ?, ?) RETURNING id')
...
route('DELETE', '/api/bank-accounts/:id', async (req, res, body, params) => {
  await db.prepare('DELETE FROM saved_bank_accounts WHERE id = ?').run(params.id);
  sendJSON(res, 200, { deleted: true });
});
...
route('POST', '/api/wallet/:ownerType/:ownerId/withdraw', async (req, res, body, params) => {
  ...
  const alreadySaved = await db.prepare('SELECT 1 FROM saved_bank_accounts WHERE owner_type = ? AND owner_id = ? AND bank_acct = ?').get(ownerType, decodedId, bankDetails.bankAcct);
  const savedCount = (await db.prepare('SELECT COUNT(*) c FROM saved_bank_accounts WHERE owner_type = ? AND owner_id = ?').get(ownerType, decodedId)).c;
...
route('GET', '/api/admin/user-lookup/:email', async (req, res, body, params) => {
  ...
  const reseller = await db.prepare('SELECT * FROM resellers WHERE user_email = ?').get(email);
  const identity = await db.prepare('SELECT * FROM verified_identities WHERE email = ?').get(email);
  const abandonedRegistration = !!(identity && !reseller);

  const customerTxns = await db.prepare('SELECT * FROM wallet_transactions WHERE owner_type = ? AND owner_id = ? ORDER BY created_at DESC LIMIT 50').all('customer', email);
  const resellerTxns = reseller ? await db.prepare('SELECT * FROM wallet_transactions WHERE owner_type = ? AND owner_id = ? ORDER BY created_at DESC LIMIT 50').all('reseller', String(reseller.id)) : [];

  const ordersAsBuyer = await db.prepare('SELECT * FROM orders WHERE buyer_email = ? ORDER BY ordered_at DESC LIMIT 50').all(email);
  const ordersAsSeller = reseller ? await db.prepare('SELECT * FROM orders WHERE reseller_id = ? ORDER BY ordered_at DESC LIMIT 50').all(reseller.id) : [];
...
// Connecting to Postgres happens over the network, unlike SQLite's instant
// local file open — so schema setup must finish and be awaited BEFORE the
// server starts accepting any requests, or an early request could hit
// routes before tables exist.
(async () => {
  await db.initDb();
  server.listen(PORT, () => {
    console.log(`GoldComfort running on http://localhost:${PORT}  (PostgreSQL backend)`);
